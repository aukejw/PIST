...
\begin{figure}
	\center
	\includegraphics[width=.8\textwidth]{images/relevant_avg.pdf}
...
population changes over time for learning a holonomic walk. The fitness
increases up to about the first 25 generations. 

The graph in figure \ref{fig:max} shows the fitness of the fittest individual
in the population over time, which is monotonically increasing. Reason for this
is that the surviving group of each population, of which the fittest individual
...
The second experiment shows a more steady increase in performance. This can be explained by the complexity of the task: The chance that a mutation improves performance in just the task of walking forward is higher than it improving performance in multiple subtasks, all of which conribute to the total fitness of the organism. The fitness function makes it so that performance in the second experiment can not be directly compared to the first experiment, however, the respective relative increases in performance can be compared. 
...
\begin{figure}
	\center
	\includegraphics[width=.7\textwidth]{images/fitness}
	\caption{Average fitness of the population over time, learning a holonomic
	walk}
	\label{fig:avg}
\end{figure}
...
\begin{figure}
	\center
	\includegraphics[width=.7\textwidth]{images/max_fitness}
...
%Something about not re-evaluating preventing us from increasing average
%fitness
